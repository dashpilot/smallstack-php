...
class Blade
{
  ...
  public function __construct()
  {
    ...
    $this->setViewFolder("../modules" . DIRECTORY_SEPARATOR);
    $this->setCacheFolder("../modules/cache" . DIRECTORY_SEPARATOR);
  ...
  public function createCacheFolder()
  {
    if (!is_dir($this->cacheFolder)) {
      try {
        ...
        throw new \Exception(
          "Unable to create view cache folder: " . $this->cacheFolder
        );
      } catch (\Exception $e) {
        throw new \Exception(
          "Unable to create view cache folder: " . $this->cacheFolder
      ...
      function ($match) {
        ...
        if (isset(self::$directives[$match[1]])) {
          if (
            isset($match[3][0]) &&
            "(" === $match[3][0] &&
            (isset($match[3][strlen($match[3]) - 1]) &&
  ...
  protected function prepare($view)
  {
    ...
    $actual =
      $this->viewFolder . DIRECTORY_SEPARATOR . $view . $this->fileExtension;
